...
// It will also remove all duplicates by creating another new array using the spread operater and spreading a Set.
...
export const getEvents = async () => {
    ...
    if (token) {
        removeQuery();
        const url = 'https://s8s0myl3la.execute-api.us-east-1.amazonaws.com/dev/api/get-events' + '/' + token;
        const response = await fetch(url);
        const result = await response.json();
        if (result) {
            NProgress.done();
            localStorage.setItem('lasEvents', JSON.stringify(result.events));
